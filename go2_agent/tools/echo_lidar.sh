...
echo "[GO2][LIDAR] read-only PointCloud2 sample probe"
...
while IFS= read -r line; do
  ...
  found=$((found + 1))
  echo
  echo "===== PointCloud2: $topic ====="
  echo_once "$topic" || true
done <<<"$typed_topics"

if [ "$found" -eq 0 ]; then
  echo "[GO2][LIDAR] no sensor_msgs/msg/PointCloud2 topic found"
fi

echo
echo "[GO2][LIDAR] pointcloud2_topic_count=$found"
